...
type AtemDskStateOrUndefined = AtemDskState | {
    defined: boolean
}
...
export class AtemModuleDsk extends AtemSubModule {
    ...
    update(status: AtemState, pathToChange: string[]): Promise<void> {
        ...
        dskNames.forEach((key, i) => {
            ...
            if(dsk.onair != nextDsk.onair || dsk.running != nextDsk.running || dsk.tie != nextDsk.tie) {
                const answer = {
                    dsk: key,
                    onair: nextDsk.onair,
            ...
            if(dsk.rate != nextDsk.rate) {
                this.parent.runEventHandlers('dsk-rate', {dsk: key, rate: nextDsk.rate})
            }
    ...
    onRate(handler: ((param: {dsk: 'dsk1' | 'dsk2' | 'dsk3' | 'dsk4', rate: number}) => void)) {
        this.parent.registerEventHandler('dsk-rate', handler)
    }

    onState(handler: ((param: {dsk: string, running: boolean, onair: boolean, tie: boolean}) => void)) {
