...
export class AtemModuleMixer extends AtemSubModule {
    ...
    update = (status: AtemState, pathToChange: string[]): Promise<void> => {
        ...
        const next = {pgm, pvw, aux, transPrv, transPos, transRun, style, transRate, black, blackAuto}
        ...
        if(current.style != style || current.transPrv != transPrv) {
            this.parent.runEventHandlers('trans-settings', {style, preview: transPrv})
        }
    ...
    onTransitionSettings(handler: ((param: {style: string, preview: boolean}) => void)) {
    ...
    pgm = async (params: {input: string} | string) => {
        ...
        const c = new Commands.ProgramInputCommand(ME, this.parent.getSourceNumber(input))
        return this.client.sendCommand(c)
    }
    ...
    prv = async (params: {input: string} | string) => {
        ...
        const c = new Commands.PreviewInputCommand(ME, this.parent.getSourceNumber(input))
        return this.client.sendCommand(c)
    }
    ...
    aux = async (params: {input: string} | string) => {
        ...
        const c = new Commands.AuxSourceCommand(AUXBUS, this.parent.getSourceNumber(input))
        return this.client.sendCommand(c)
    }
