...
export class MacroStep {
    ...
    constructor(stepNumber: number, name: string, duration: number, trigger: tTrigger, actions: MacroActionDefinition[], macro: Macro) {
        this.name = name
        ...
        this.actions = actions.map(action => {
            return new MacroAction(action.mod, action.modName, action.path, action.params, this)
        })

        const actionDuration = this.actions.reduce((sum, action) => sum + action.getDuration(), 0)
    ...
    reset() {
        ...
        if(this.running) {
            console.log(colors.yellow(`[STEP] ${this.macro.executor} STEP ${this.stepNumber} CANCELLING ACTIONS`))
    ...
    getExecuter() {
        ...
        return () => {
            return promiseChain(false, this.actions.map((action, index) => {
        ...
        return () => {
            return new Promise<boolean>((resolve, _reject) => {
                ...
                if(this.onCancel && cancelled) {
                    console.log(colors.yellow(`[STEP] ${this.macro.executor} STEP ${this.stepNumber} TRIGGER-TIMER CANCELLED`))
                }
