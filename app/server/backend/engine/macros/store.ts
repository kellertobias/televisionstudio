...
export class MacroStore {
    ...
    async init() {
        this.configFilesLocation = this.config.generic.showfile || this.config.showfile
        this.file = fs.open(`${this.configFilesLocation}`, 'a+', 0o666).catch(err => {
            console.log(err)
            ...
            (macroObj.steps || []).map((step : any, i: number) => {
                return {
                    ...
                    actions: (step.actions || []).map((action : any) => {
                        ...
                        const verifiedModuleName : ModuleNames = moduleName.toLowerCase() as ModuleNames
    ...
    load() {
        ...
        return this.file.then(handle => {
            if(!handle) return Promise.resolve(undefined)
            return handle.readFile(defaultEncoding)
        }).then((contents) => {
            try {
                if(!contents) {
                    console.log(colors.red.bold("[SHOWFILE] Could not Load: Showfile is empty"), contents)
                ...
                console.log(colors.yellow.bold(`[SHOWFILE] YAML File Loaded. Checking Structure…`))
                if(typeof obj !== 'object') {
                    console.log(colors.red.bold(`[SHOWFILE] Could not Load: Structure is Wrong`))
                    throw Error('Showfile: Must be Object on Top Level')
                }

                if(!Array.isArray(obj.macros)) {
                    console.log(colors.red.bold(`[SHOWFILE] Could not Load: Macros missing`))
                ...
                if(obj?.obs?.scenefile) {
                    ...
                    console.log(colors.yellow.bold(`[SHOWFILE] No OBS Scene Collection assigned`))
                }
                ...
                if(!mark) {
                    console.log(colors.red.bold(
                        `[SHOWFILE] Error Parsing Macro File: ${error}`
                    ))
                } else {
                    console.log(colors.red.bold(
