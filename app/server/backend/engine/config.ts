...
export class ConfigBackend {
    ...
    devices : {
        ...
        deskleft: DeviceParameters,
        deskright: DeviceParameters,
    ...
    async listShowFiles(subpath: string[]) {
        return new Promise((resolve, reject) => {
            ...
            fs.readdir(searchpath, { withFileTypes: true }).then((files) => {
                resolve(files.map(f => {
    ...
    private openFile(name: string, mode?: string) {
        ...
        console.log(`[CONFIG] Open File: ${this.configFilesLocation}/${name}.json as ${name}`)

        return fs.open(`${this.configFilesLocation}/${name}.json`, mode, 0o666).catch(err => {
            console.error(err)
    ...
    async _loadFile(varName: "devices" | "generic") {
        ...
        return this.files[varName].then(handle => {
    ...
    load() {
        return Promise.all([
            ...
            console.log("[CONFIG] LOADED", {devices: this.devices, generic: this.generic})
            return Promise.resolve()
        })
    }
