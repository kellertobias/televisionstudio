...
export class ObsModuleOutput extends ObsSubModule {
    ...
    _update() {
        ...
        return this.getStreamSettings().then((params) => {
            if(params) {
                const {server, key} = params
                if(this.streamingServer?.server != server || this.streamingServer?.key != key) {
                    serverChanged = true
            ...
            if(streaming != this.streaming || this.streamTime != streamTime || serverChanged) {
    ...
    onStreamChanged(handler: ((params: {
        status: liveStatus,
        time: number,
        skipped: number,
        bandwidth: number,
    ...
    onRecordingChanged(handler: ((params: {
        status: liveStatus,
        time: number
    }) => void)) {
    ...
    setFolder = async (params: {folder: string} | string) => {
        ...
        return this.client.send('SetRecordingFolder', {
            'rec-folder': folder
        }).then(() => {
            this.folder = folder
        })
